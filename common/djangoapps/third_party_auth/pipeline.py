...
def parse_query_params(strategy, response, *args, **kwargs):
    ...
    # TODO eduNEXT: hotfix when request.session has a broken auth_entry (None value). This fix shouldn't be
    # supported in incoming version.
    auth_entry = strategy.request.session.get(AUTH_ENTRY_KEY, AUTH_ENTRY_LOGIN) or AUTH_ENTRY_LOGIN
